...
UPPSALA_SE_FILE <- file.path( "data", "objects", "UPPSALA_se.rds" )

## figures
KMPLOT_MISCLASS_STAGEIII_FILE <- file.path( "figures", "kmplot_misclass_stageIII.pdf" )
COXPHMIC_PLOT_FILE <- file.path( "figures", "coxphMIC_plot.pdf" )
COXPHMIC_HEATMAP_FILE <- file.path( "figures", "coxphMIC_heatmap.pdf" )
COXPHMIC_11_GENES_BOXPLOT_TCGA_FILE <- file.path( "figures", "coxphMIC_11_genes_boxplots_TCGA.pdf" )
KMPLOT_RAGx_RAG_0_OS_TCGA_FILE <- file.path( "figures", "kmplot_RAGx_RAG-0_OS_TCGA.pdf" )
KMPLOT_RAS_INDEX_OS_TCGA_FILE <- file.path( "figures", "kmplot_ras_index_OS_TCGA.pdf" )
KMPLOT_RAS_INDEX_KRASMUT_OS_TCGA_FILE <- file.path( "figures", "kmplot_ras_index_krasmut_OS_TCGA.pdf" )
VOLCANO_UVCOX_PARENT_SIGS_TCGA_FILE <- file.path( "figures", "volcano_uvcox_parent_sigs_TCGA.pdf" )

COXPHMIC_PLOT_UPSALA_FILE <- file.path( "figures", "coxphMIC_plot_UPPSALA.pdf" )
UPPSALA_PARENT_SIGNATURE_OVERLAP_HEATMAP_FILE <- file.path( "figures", "uppsala_parent_siganture_class_overlap_heatmap.pdf" )
KMPLOT_RAS_INDEX_UPPSALA_FILE <- file.path( "figures", "kmplot_ras_index_uppsala.pdf" )
FOREST_PLOT_COXPH_MULTIVAR_UPPSALA_FILE <- file.path( "figures", "forestplot_coxph_multivar_uppsala.pdf" )
COXPH_MULITVAR_PARENT_SIG_VOLCANO_PLOT_UPPSALA_FILE <- file.path( "figures", "coxph_multivar_parent_sig_volcano_uppsala.pdf" )
```
...
cairo_pdf( file = KMPLOT_MISCLASS_STAGEIII_FILE, width = 4, height = 4)
print( gg )
dev.off()
```
...
cairo_pdf( file = COXPHMIC_PLOT_FILE, width = 5, height = 3 )
print( plot( fit.mic ) )
dev.off( )
...
cairo_pdf( file = COXPHMIC_HEATMAP_FILE, width = 8, height = 3 )
print( hm )
dev.off( )
```
```{r MIC_RI}
...
cairo_pdf( file = COXPHMIC_11_GENES_BOXPLOT_TCGA_FILE, width = 2, height = 3 )
print( bw_gg )
dev.off()
```
...
for( i in levels(  se_t$RAS84 )[ -1 ] ) {
    ...
    print( gg )
    cairo_pdf( file = sub( "RAGx", i, KMPLOT_RAGx_RAG_0_OS_TCGA_FILE ),
              width = 4, height = 4 )
    print( gg )
    dev.off()
    cat( " \n\n" )
...
print( gg )
cairo_pdf( file = KMPLOT_RAS_INDEX_OS_TCGA_FILE, width = 4, height = 4 )
print( gg )
dev.off( )
```
...
cairo_pdf( file = KMPLOT_RAS_INDEX_KRASMUT_OS_TCGA_FILE, width = 5, height = 5 )
print( gg )
dev.off( )
```
...
parent_sigs <- c( "HRAS", "KRASG13D134", "KrasLA", "MSigDB", "RAS_pathway" )
...
print( mv_surv_gg )
cairo_pdf( file = VOLCANO_UVCOX_PARENT_SIGS_TCGA_FILE, width = 5, height = 4 )
print( uvsurv_gg )
dev.off( )
```
...
se_uppsala <- readRDS( file = UPPSALA_SE_FILE )
```
...
cairo_pdf( file = COXPHMIC_PLOT_UPSALA_FILE, width = 5, height = 3 )
print( plot( fit.mic ) )
dev.off( )
...
cairo_pdf( file = UPPSALA_PARENT_SIGNATURE_OVERLAP_HEATMAP_FILE, width = 4 )
print( hm )
dev.off( )
```
...
cairo_pdf( file =KMPLOT_RAS_INDEX_UPPSALA_FILE, width = 4, height = 4 )
print( gg )
dev.off( )
```
...
cairo_pdf( file = FOREST_PLOT_COXPH_MULTIVAR_UPPSALA_FILE, width = 6, height = 3 )
print( gg )
dev.off( )
```
...
print( mv_surv_gg )
cairo_pdf( file = COXPH_MULITVAR_PARENT_SIG_VOLCANO_PLOT_UPPSALA_FILE, width = 5, height = 4 )
print( mv_surv_gg )
dev.off( )
```
```{r output_results}

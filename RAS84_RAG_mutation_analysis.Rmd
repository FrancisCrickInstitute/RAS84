...
adat <- project.init( file.path( tumour, sig_n ), root = "./" )
```
...
```{r data_in}
se_t_file <- paste0( "se_t_", sig_n, "_rasclass.rda" )
se_t_file <- file.path( "..", "..", "sample_classification", "TCGA", tumour, se_t_file )
load( file = se_t_file )
assay( se_t ) <- round( assay( se_t ) )

load( file = file.path( "..", "..", "validate_signatures", "TCGA", "LUAD", "signatures.rda" ) )

sig_name_map <- c( Loboda = "RAS pathway",
                  Downward_146 = "KRASG13D134",
                  Bild.Nevins = "HRAS",
                  KRAS_msigdb_HM = "MSigDB",
                  SweetCordero = "KrasLA",
                  Downward_68 = "KRASG13D68",
                  Singh_Settleman = "RAS addiction",
                  RAS84 = "RAS84" )
...
# RAS activity group characterisation

Here we produe a heatmap highlighting the alteration characterisation
of the five RAS activity groups.

```{r rasact_charact}
df <- data.frame( KRAS = c( -1, +1, 0, 0 ,0 ),
                 EGFR = c( 0, -1, +1, +1, -1 ),
                 STK11 = c( 0, +1, -1, -1, 0 ),
                 KEAP1 = c( +1, +1, -1, -1, 0 ),
                 RB1 = c( +1, 0, 0, 0, 0 ),
                 ...
                 CTNNB1 = c( +1, 0, 0, 0, 0 ),
                 CDKN2A = c( 0, 0, 0, 0, +1 ),
                 row.names = levels( se_t$RAS84 ) )

hm <- Heatmap( as.matrix( df ),
              cluster_rows = FALSE,
              cluster_columns = FALSE,
              rect_gp = gpar(col = "grey" ),
              col = c( my_blue, "white", my_red ),
              show_heatmap_legend = FALSE,
              row_split = c( 1,2,3,4,5 ) )
print( hm )
pdf( file = file.path( adat$plot.path, "sample_class_heatmap.pdf" ),
    height = 3, width = 4 )
print( hm )
dev.off( )
```

# Small signature KRAS mutation analysis
...
small_sigs_RF_ranked <- map( 5:nrow( RF_genes_ranked ), function( i ) {
    RF_genes_ranked$genes[ 1:i ]
} )
names( small_sigs_RF_ranked ) <- as.character( 5:nrow( RF_genes_ranked ) )
...
print( gg )
dev.off( )
png( file = file.path( adat$plot.path, paste0( sig_n, "_rasact_KRASmut_type.png" ) ),
...
chisq_dat <- data.frame( RAS84 = se_t$RAS84,
                         mut = se_t$MUT.STK11 )
chisq.test( table( chisq_dat ) )$p.value
```

# RAS-Pathway Misclassified Sample analysis
...
mutation_dat <- colData( se_t ) %>%
    ...
    dplyr::select(
               ...
               contains( "MUT", ignore.case = FALSE ) ) %>%
               #contains( "FUSION", ignore.case = FALSE ) ) %>%
    tidyr::gather( variants, value, -barcode, -RAS84, -Loboda_classified )
...
mutation_tab <- mutation_dat %>%
    group_by( RAS84, Loboda_classified, variants ) %>%
    dplyr::summarize( mutant = sum( value ),
                      wildtype = sum( !value ) ) %>%
    gather( genotype, count, -RAS84, -Loboda_classified, -variants ) %>%
    unite( colheader, RAS84, genotype, sep = "." ) %>%
    spread( colheader, count )
...
mutation_res <- mutation_dat %>%
    ...
    map( function( df ) {
        l <- df %>%
            droplevels( ) %>%
            split( .$variants )
        res <- l %>% map( function( l_df ) {
            l_df %>%
                dplyr::select( Loboda_classified, value ) %>%
                table( ) %>%
                chisq.test()
        } )
        f <- map( res, function( chisqres ) chisqres$p.value ) %>%
            unlist( ) < 0.05
        res[ f ]
    } )
names( mutation_res ) <- levels( mutation_dat$RAS84 )
...
mutation_dat <- colData( se_t ) %>%
    as.data.frame( ) %>%
    mutate( RAS_high_low_RAS84 = factor( ifelse( RAS84 == "RAG-0", "ras_low", "ras_high" ) ) ) %>%
    mutate( RAS_high_low_RAS_PW = factor( ifelse( Loboda == "RAG-0", "ras_low", "ras_high" ) ) ) %>%
    mutate( class_correct = RAS_high_low_RAS84 == RAS_high_low_RAS_PW )

tab <- mutation_dat %>%
    filter( RAS_high_low_RAS84 == "ras_low" ) %>%
    dplyr::select( MUT.KRAS, class_correct ) %>%
    table( )

chisq_res <- mutation_dat %>%
    group_split( RAS_high_low_RAS84 ) %>%
    map( function( df ) {
        list( tumour_stage = df %>%
                  dplyr::select( class_correct, tumor_stage ) %>%
                  table( ) %>%
                  chisq.test(),
             gender = df %>%
                 dplyr::select( class_correct, gender ) %>%
                 table( ) %>%
                 chisq.test(),
             race = df %>%
                 dplyr::select( class_correct, race ) %>%
                 table( ) %>%
                 chisq.test() )
    } )
names( chisq_res ) <- levels( mutation_dat$RAS_high_low_RAS84 )

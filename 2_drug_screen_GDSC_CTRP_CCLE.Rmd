...
## Paths
GDSC_SE_L_FILE <- file.path( "data", "gdsc_se_l.rds" )
...
RAS_PATHWAY_DEFINITION_FILE <- file.path( "data", "downloads", "1-s2.0-S0092867418303593-mmc3.xlsx" )

ESET_CCLE_AFFY_LUNG_FILE <- file.path( "data", "objects", "eset_ccle_affy_lung.rds" )
RAS84_HIGH_LOW_GDSC_CCLE_SIG_COMPOUNDS_FILE <- file.path( "data", "RAS84_HighvLow_CCLE_GDSC_sig_compounds.xlsx" )

## figures
GDSC_KRASMUT_VOLCANO_FILE <- file.path( "figures", "gdsc_ccle_krasmut_volcano.pdf" )
GDSC_RASPATHWAY_VOLCANO_FILE <- file.path( "figures", "gdsc_ccle_raspathway_volcano.pdf" )
GDSC_ENRICHMENT_PLOT_FILE <- file.path( "figures", "gdsc_ccle_enrichment_plot.pdf" )
GDSC_RESISTANT_ENRICHMENT_PLOT_FILE <- file.path( "figures", "gdsc_ccle_resistant_enrichment_plot.pdf" )
GDSC_VOLCANO_PLOT_FILE <- file.path( "figures", "gdsc_volcano.pdf" )
```
...
```{r gdsc_drug_data}
gdsc_se_l <- readRDS( GDSC_SE_L_FILE )
```
...
```{r setup_se}
rasPathwayTab <- read.xlsx( RAS_PATHWAY_DEFINITION_FILE, sheet = 8, check.names = TRUE )
...
rnaseq_se <- readRDS( ESET_CCLE_AFFY_LUNG_FILE )
```
...
cairo_pdf( file = GDSC_KRASMUT_VOLCANO_FILE,
           width = 4, height = 4)
print( vol_gdsc_krasmut_gg )
dev.off( )
```
...
cairo_pdf( file = GDSC_RASPATHWAY_VOLCANO_FILE, width = 4, height = 4)
print( vol_gg )
dev.off( )
```
...
cairo_pdf( file = GDSC_ENRICHMENT_PLOT_FILE, width = 7, height = 3 )
print( enrich_gg   )
dev.off( )
...
cairo_pdf( file = GDSC_RESISTANT_ENRICHMENT_PLOT_FILE, width = 7, height = 2.5 )
...
cairo_pdf( file = GDSC_VOLCANO_PLOT_FILE )
print( vol_gg )
dev.off( )
```
...
dbase <- map( drugs_decide_l[ 1:2 ], bind_rows ) %>%
    ...
    dplyr::select( -key )
write.xlsx( dbase, RAS84_HIGH_LOW_GDSC_CCLE_SIG_COMPOUNDS_FILE )
```

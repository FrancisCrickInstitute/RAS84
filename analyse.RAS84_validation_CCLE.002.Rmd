...
```{r CCLE_init,message=FALSE,warning=FALSE}
eSetFile <- file.path( "..", "..", "..", "..", "analysis", "CCLE", "eSet.rda" )
load( file = eSetFile )
eset_ccle <- eSet

eset_ccle_lung <- eset_ccle[ , pData( eset_ccle )$tumour == tumour ]
...
colnames( ov ) <- sig_name_map[ colnames( ov ) ]

hm <- Heatmap( ov, col = c( "white", my_purple ),
              show_heatmap_legend = FALSE,
              column_split = c( 1,1,1,1,2,1 ) )
print( hm )
if( !file.exists( pdf_file <- file.path( adat$plot.path, "sig_ov_heatmap.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 1.75, height = 5 )
    print( hm )
    dev.off( )
}
```
...
```{r ras_pathway,message=FALSE,warning=FALSE}
...
```{r RAS_annotate,message=FALSE,warning=FALSE}
...
print( gg1 )
if( !file.exists( pdf_file <- file.path( adat$plot.path, "oncogenic_ras_sample_counts.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 1.3, height = 3 )
    print( gg1 )
    dev.off( )
}
```
...
```{r sig_assess,message=FALSE,warning=FALSE}
...
plotdat$signature <- factor( sig_name_map[ as.character( plotdat$signature ) ],
                            levels = c( "HRAS", "KRASG13D134", "KrasLA", "RAS pathway", "MSigDB", "RAS addiction" ) )
...
print( gg )
if( !file.exists( pdf_file <- file.path( adat$plot.path, "sig_gene_selection.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 3*1.5, height = 2*1.5 )
    print( gg )
    dev.off( )
}
```
...
figure. This is for the paper.

```{r add_ras84}
ras84_feature_map <- read.xlsx( file.path( "..",   "..", "..", "..", "data", "signatures", "ras", "RAS84_feature_map.xlsx" ) )
...
```{r cluster,message=FALSE,warning=FALSE,results='asis',echo=FALSE}
...
for( sig_n in names( signatures ) ) {
    ...
    pdf_file <- file.path( adat$plot.path, "heatmaps", paste( sig_n, "heatmap.ward.pdf", sep = "_" ) )
    cairo_pdf( file = pdf_file, width = 2, height = 3 )
    print( hm3d + hm_anno_row )
    dev.off( )
    if( sig_n == "RAS84" ) {
        if( !file.exists( pdf_file <- file.path( adat$plot.path, "heatmaps", paste( sig_n, "large_heatmap.ward.pdf", sep = "_" ) ) ) ) {
            cairo_pdf( file = pdf_file, width = 3, height = 4 )
            print( hm3d + hm_anno_row )
            dev.off( )
        }
    }
...
pdf_file <- file.path( adat$plot.path, "ras_highlow_contingency.pdf" )
cairo_pdf( file = pdf_file, height = 3, width = 6.5 )
...
```{r ras84,message=FALSE,warning=FALSE}
hmcols <- colorpanel( 1000, my_blue, "white", my_red )

RAS84_parent_signatures <- c( "Loboda", "Bild.Nevins", "Downward_134" )
...
print( hm3d + hm_anno_row )
if( !file.exists( pdf_file <- file.path( adat$plot.path, "heatmaps", "RAS84_raw_heatmap.ward.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 2, height = 3 )
    print( hm3d + hm_anno_row )
    dev.off( )
}
...
write.table( signatures$RAS84, file = file.path( "data", "signatures", "ras", "curated", "RAS84.csv" ),
            row.names = FALSE, col.names = FALSE, quote = FALSE )
save( signatures, file = file.path( adat$analysis.path, "signatures_filtered.rda" ) )
```
...
if( !file.exists( pdf_file <- file.path( adat$plot.path, "signature_cluster_volcanos.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 3.5, height = 1.75)
    print( gg )
    dev.off( )
}
```
...
if( !file.exists( pdf_file <- file.path( adat$plot.path, "heatmaps", "RAS84_heatmap.ward.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 6, height = 4 )
    print( hm3d + hm_anno_row )
    dev.off( )
}
```
...
if( !file.exists( pdf_file <- file.path( adat$plot.path, "signature_volcano_yes_RAS84.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 5, height = 4 )
    gg1d_all_sigs
    dev.off( )
}
...
cairo_pdf( file = file.path( adat$plot.path, "KRAS_mutation_pvalue.pdf" ), width = 3, height = 3 )
...
```{r controls}
control_sig_home <- file.path( "data", "signatures", "controls" )
control_sig_files <- dir( path = control_sig_home, pattern = "\\.txt" )
...
gg1d_all_sigs <- gg1d_all_sigs + theme( legend.position = "right" )
print( gg1d_all_sigs )
if( !file.exists( pdf_file <- file.path( adat$plot.path, "control_signature_volcano.pdf" ) ) ) {
    cairo_pdf( file = pdf_file, width = 5, height = 4 )
    gg1d_all_sigs
    dev.off( )
}

...
eSetFile <- file.path( "data", "CCLE_Affy_eSet.rds" )
eset_ccle <- readRDS( file = eSetFile )

eset_ccle_lung <- eset_ccle[ , pData( eset_ccle )$tumour == "LUNG" ]
...
signatures_raw <- lapply( krasSigNames, function( sigFile ) {
    read.table( file = file.path( SIG_HOME, paste( sigFile, "csv", sep = "." ) ),
               sep = "\t", stringsAsFactors = FALSE )[, 1 ]
} )
names( signatures_raw ) <- make.names( krasSigNames )
```
```{r signature_overlap)
...
rasPathwayDefFile <- file.path( "data", "oncogenic_RAS_pathway_defintions_Sanchez-Vega_etal.txt" )
